...
export async function middleware(request) {
  // Whitelist health, ops, and auth endpoints - always allow
  if (request.nextUrl.pathname.startsWith('/api/health') || 
      request.nextUrl.pathname.startsWith('/api/ops/status') ||
      request.nextUrl.pathname.startsWith('/api/auth-check') ||
...
export const config = {
  matcher: [
    '/((?!api/health|api/auth-check|api/ops/status|auth|_next|favicon.ico).*)',
  ],
};
